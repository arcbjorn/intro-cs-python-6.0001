...
from helpers import get_user_input_as_int
...
def is_valid_word(word, hand, word_list):
    ...
    def convert_wildcards_to_words(word):
        list_of_word = []
        for vowel in VOWELS:
    ...
    if '*' in word:
        words = convert_wildcards_to_words(word)
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(word, hand, word_list):
                ...
                total_score += points
                print(
                    f'\"{word}\" earned {points} points. Total: {total_score} points')
...
def substitute_hand(hand, letter):
    ...
    curr_letter_items = hand_copy[letter]
    del(hand_copy[letter])

    if letter in VOWELS:
        chosen_list = VOWELS
    else:
        chosen_list = CONSONANTS

    while True:
        new_letter = random.choice(chosen_list)
        if new_letter not in hand.keys():
            hand_copy[new_letter] = curr_letter_items
            break
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    score = 0
    answer = 'no'
    total_hands_n = get_user_input_as_int('Enter total number of hands: ')
    for hands_n in range(total_hands_n):
        if hands_n > 0:
            answer = input(
